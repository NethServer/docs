...
* System user, certificate, one time password (2FA)

.. note::
 Two-factor authentication (2FA) can be used to add an extra layer of security required to access the roadwarrior VPN. 
 All users will be forced to use the one time password policy, they have to create their 2FA secrets themselve at  :ref:`2fa-section`.
 Users will enter user name and a temporary verification code generated by an application running on their smartphone.
...
.. warning::
 Due to the temporary code of 2FA, the password in the VPN client must not be saved, it will be changed after 30 seconds.
 A password saved inside a VPN client can be seen as a login failure by  :ref:`fail2ban-section`
